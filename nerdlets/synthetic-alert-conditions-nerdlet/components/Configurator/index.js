...
export default class SchemaForm extends React.Component {
    ...
    static propTypes = {
        ...
        enableToolsPanel: PropTypes.bool
    }
    ...
    saveToStorage(data) {
    /*
    We save one document with meta data indicating how many other documents there are to load.
    The data is then saved to multiple documents of 60k size each
    */

        const chunkSize= 60000 //60k ish
    ...
    loadFromStorage() {
        ...
        AccountStorageQuery.query({
            ...
            if(metaData) {
                ...
                const hasDefault = this.props.default;
                Toast.showToast({
                    type: hasDefault ? Toast.TYPE.NORMAL : Toast.TYPE.CRITICAL,
                    title: "No config data",
                    description: "No previously saved config data was found, initialising with " + hasDefault ? "default" : "empty" + " config.",
                    sticky: false
                });
                if (hasDefault) {
    ...
    render() {
        ...
        if (typeof this.props.children!== 'undefined'){
            if (!this.state.dataLoaded || this.state.dataLoaded === false){
                return(<div>
                     <HeadingText type={HeadingText.TYPE.HEADING_4}>Loading Account Configurations<Spinner type={Spinner.TYPE.DOT}  inline/></HeadingText>
                     </div>);

            }else{
                return ( React.Children.map( this.props.children, (child) => (React.cloneElement(child, {accountAPIKeys:this.state.persistentData}))) );
            }
        ...
        if(configVisible) {
            ...
            if(dataLoaded) {
                ...
                let form = <Form schema={schema}
                        ...
                        onError={log("errors")}
                        >
                        <button className="u-unstyledButton btn btn-info" type="submit"><Icon spacingType={[Icon.SPACING_TYPE.SMALL]} type={Icon.TYPE.HARDWARE_AND_SOFTWARE__SOFTWARE__UPSTREAM_CONNECTION} inline />Save</button>
                    </Form>

                    returnVal=<>
                    <div className="ConfiguratorForm">{form}</div>
                    {
                         this.props.enableToolsPanel   && <ToolPanel previousData="test" currentData={persistentData} saveHandler={this.saveJSONdata} documentId={documentId} storageCollectionId={storageCollectionId}/>
                    }
                </>
            }
         ...
         return <div>
            <button className="btn btn-primary u-unstyledButton" onClick={()=>{this.setState({configVisible: !configVisible})}}>
